...
void init_neuron(neuron* neuron, int n_parameters) {
    // neuron->inputs = malloc(n_parameters * sizeof(float));
    neuron->weights = malloc(n_parameters * sizeof(float));
    neuron->n_weights = n_parameters;

    float limit = sqrt(2.0 / n_parameters);
...
layer dense(int n_neurons, int n_parameters, function *activation_function) {
    ...
    layer.outputs = malloc(n_neurons * sizeof(float));
    layer.raw_outputs = malloc(n_neurons * sizeof(float));
    layer.derivatives = malloc(n_neurons * sizeof(float));
...
void train(MLP *m, float* raw_inputs, float* target, float lr) {
    ...
    for (int i = 0; i < l->n_neurons; i++) {
        // float output = l->neurons[i].output;
...
void save_model(MLP* m, const char* path) {
    ...
    for (int i = 0; i < m->n_layers; i++) {
        ...
        for (int j = 0; j < l->n_neurons; j++) {
            ...
            fwrite(n->weights, sizeof(float), n->n_weights, f);
...
void load_model(MLP* m, const char* path) {
    ...
    for (int i = 0; i < m->n_layers; i++) {
        ...
        l->derivatives = NULL; // malloc(l->n_neurons * sizeof(float));
        ...
        for (int j = 0; j < l->n_neurons; j++) {
            ...
            n->weights = malloc(sizeof(float) * n->n_weights);

            // Read weights and bias
            fread(n->weights, sizeof(float), n->n_weights, f);
            fread(&n->bias, sizeof(float), 1, f);
        }
